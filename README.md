...
![Tests](https://img.shields.io/badge/tests-27_passing-2ea44f)
...
|---|---|
| Is the member ID present and in our eligibility list? | Read a fax document into structured fields |
| Is the provider's NPI mathematically valid? *(real checksum)* | Do these clinical notes actually satisfy the policy? |
| Is the diagnosis code a real ICD-10 format? | Write the follow-up letter to the provider |
| Is this a duplicate of a recent request? | *(that's the whole list)* |
...
The dataset is generated by a script that deliberately breaks things: **27 packets and 2 faxes across 18 scenarios**. Because the script *creates* each problem, it also knows the right answer, and writes it to `data/golden_labels.json`.
...
Plus **27 automated tests** covering the NPI checksum, routing priorities, fax extraction, duplicate logic, and the budget guard.
